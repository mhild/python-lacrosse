...
MAX_AGE_IN_S = os.environ.get("MAX_AGE_IN_S", 600)
...
class Config(object):
    ...
    def get_sensor(self, id=None):
        ...
        return self._filter_unknown_max_age()

    def _filter_unknown_max_age(self):
        ...
        filtered_cfg = {
            ...
            if (
                ('name' in v and v['name'] is not None)
                or (
                    'lastseen' in v
                    and (now - dtparser.parse(v['lastseen'])).total_seconds()
                    > MAX_AGE_IN_S
                )
            )
        }
    ...
    def remove_orphaned_entries(self):
        ...
        for id in self.config.keys():
            if "lastseen" not in self.config[id]:
                continue
    ...
    def update_state(self, sensor: LaCrosseSensor):
        ...
        if sensor.sensorid in self.config.keys():
            ...
            self.config[sensor.sensorid][
                "lastseen"
            ] = f"{datetime.now(tz=pytz.timezone(TIMEZONE)):%Y-%m-%dT%H:%M:%S%z}"
